...
|--------|------|--------|
| `bannedWords` | 违禁词列表 | `[]` |
| `bannedWordsAction` | 处理方式: `reject`(拒绝) / `replace`(替换) | `reject` |
| `msgBanned` | 触发违禁词提示语 | `你的描述包含敏感内容，无法生成喵` |

**处理方式说明：**
- `reject`: 检测到违禁词时直接拒绝生成，返回提示语
- `replace`: 将违禁词替换为 `*` 后继续生成图片
...
~zimage:
  ...
  bannedWordsAction: reject
  msgBanned: '你的描述包含敏感内容，无法生成喵'
  # 提示语配置
  msgGenerating: '在画了喵'
  msgError: '发生错误了喵'
...
检查配置中的 `bannedWords` 是否正确设置，或使用命令 `画图违禁词` 查看当前列表。
